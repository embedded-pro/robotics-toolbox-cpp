---
description: "Implement code changes in robotics-toolbox — float-only templates, no heap, embedded pragmas, TEST_F on float, CMake wiring, docs. Needs a clear task or plan."
...
#endif
```
Matching `.cpp`: `template class Algorithm<float, N>;` — add via `robotics_add_coverage_sources()`.
